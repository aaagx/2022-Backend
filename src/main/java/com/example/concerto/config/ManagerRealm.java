...
public class ManagerRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        Manager managerInfo=managerService.getManagerInfoByTel(userToken.getUsername());
        ...
        Object principal=managerInfo;
        //2).credentials:密码（数据库获取的用户的密码）
        Object credentials=managerInfo.getPassword();
        //3).realmName：当前realm对象的name，调用父类的getName()方法即可
        String realmName=getName();
        //4).盐值
        ByteSource credentialsSalt = ByteSource.Util.bytes(managerInfo.getSalt());//盐值 要唯一
